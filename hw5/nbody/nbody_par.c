/* nbody.c: sequential 2-d nbody simulation
   ...
   Compile this with a source file that defines the missing
...
static void init(char* filename) {
  ...
  printf("nthreads= %d, nbody: nbodies=%d nx=%d ny=%d nstep=%d wstep=%d\n",
	 nthreads, nbodies, nx, ny, nstep, wstep);
...
static inline void* update(void* arg) {
  int *tnum = (int*)arg;
  if(*tnum < nbodies){
  for (int i = *tnum; i<nbodies; i+=nthreads) {
  ...
  // BELOW should be done by one thread?
  //if(tnum==0){
...
static void wrapup() {
  ...
  free(states);
  free(states_new);
  printf("\nnbody: finished.  nthreads = %d, Time = %lf\n", nthreads, ANIM_time() - start_time);
}
...
int main(int argc, char * argv[]) {
  ...
  for (int i=1; i<=nstep; i++) {
    for(int n=0; n<nthreads; n++){
    	pthread_create(threads + n, NULL, update, tids + n);
    }
    for(int i=0; i<nthreads; i++)
